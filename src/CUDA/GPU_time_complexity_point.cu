...
int main()
{
	...
	FILE* document;
	fopen_s(&document, "GPU_ICP_point_to_point_TimeComp.csv", "w");
	...
	for (WIDTH = 3; WIDTH <= 128; WIDTH += 1)
	{
		...
		while (iteration < MAX_ITER)
		{
			...
			//get the deviation
			deviation << <GridSize, BlockSize >> > (num_points, d_p, d_q_idx, d_barP, d_barQ, d_devP, d_devQ);
			err = cudaGetLastError();
